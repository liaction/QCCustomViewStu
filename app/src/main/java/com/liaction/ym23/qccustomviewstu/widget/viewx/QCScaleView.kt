...
class QCScaleView @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : View(context, attrs, defStyleAttr) {
    private val bitmap = qcGetImageFromSource(230.dpx.toInt(), resources, R.drawable.mantou)
    private val bitmapMatrix = Matrix()
    override fun onDraw(canvas: Canvas) {
        canvas.scale(0.5f, 0.5f, bitmap.width / 2f, bitmap.height / 2f)
        bitmapMatrix.reset()
        bitmapMatrix.preScale(0.5f, 0.5f, bitmap.width / 2f, bitmap.height / 2f)
